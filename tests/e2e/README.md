...
| #112 | 檢核時可引用已上傳法規資料 | `upload-activate.spec.js`（上傳 → 啟用建索引 → 檢核報告引用得到） |
| — | 工具按鈕點了要真的跑起來 | `tool-buttons.spec.js` |

## 治理閘門怎麼驗

`upload-activate.spec.js` 走完整鏈路：上傳（pending，生效部數不變）→ 管理者按啟用
（就地抽條文與管制條目）→ 生效部數 +1 → 執行檢核 → 報告的逐條判定總表引用到它。
這條同時證明「未啟用不影響判定」與「啟用後真的被引用」。
