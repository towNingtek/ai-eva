...
export default function ChartAnalysis() {
  ...
  const markFiltersChanged = () => setFiltersChanged(true);
  const query = () => {
    setFiltersChanged(false);
    callAction({ name: "chart_query", payload: { year, district, sdgs } });
  };
  const exportChart = (format) => callAction({ name: "chart_export", payload: { year, district, sdgs, format } });
  const items = filtersChanged ? [] : (props.items || []);
    ...
    <div className="w-full max-w-3xl rounded-xl border bg-card p-4 text-card-foreground shadow-sm">
      <div className="mb-4 grid gap-3 sm:grid-cols-2">
        <label className="grid gap-1 text-sm">年度
          <select className="rounded-md border bg-background p-2" value={year} onChange={(e) => { setYear(e.target.value); markFiltersChanged(); }}>
            {YEARS.map((value) => <option key={value} value={value}>{value}</option>)}
          </select>
        </label>
        <label className="grid gap-1 text-sm">行政區（可留空）
          <input className="rounded-md border bg-background p-2" value={district} onChange={(e) => { setDistrict(e.target.value); markFiltersChanged(); }} placeholder="全區" />
        </label>
      </div>
      <div className="mb-4 flex flex-wrap gap-2">
        {SDGS.map(([id, label]) => <button key={id} className={`rounded-full border px-3 py-1 text-xs ${sdgs.includes(id) ? "bg-primary text-primary-foreground" : "bg-background"}`} onClick={() => { toggleSdg(id); markFiltersChanged(); }}>{label}</button>)}
      </div>
      <div className="mb-4 flex flex-wrap gap-2">
        ...
        <button className="rounded-md border px-4 py-2 text-sm" onClick={() => exportChart("pdf")} disabled={!sdgs.length || props.loading || filtersChanged}>匯出 PDF</button>
        <button className="rounded-md border px-4 py-2 text-sm" onClick={() => exportChart("png")} disabled={!sdgs.length || props.loading || filtersChanged}>匯出 PNG</button>
      </div>
      {filtersChanged && <p className="mb-3 text-sm text-muted-foreground">條件已變更，請先按「生成圖表」再匯出。</p>}
      {props.error && <p className="mb-3 text-sm text-destructive">{props.error}</p>}
      {items.length ? <div className="space-y-3" aria-label="SDG 預算長條圖">
        {items.map((item, index) => <div key={item.id} className="grid grid-cols-[4rem_1fr_8rem] items-center gap-2 text-sm"><span>{item.id.toUpperCase()}</span><div className="h-7 rounded bg-muted"><div className="h-7 rounded" style={{ width: `${Math.max(2, (item.value / max) * 100)}%`, backgroundColor: COLORS[index % COLORS.length] }} /></div><span className="text-right">NT$ {(item.value || 0).toLocaleString()}</span></div>)}
